...
def on_click():
    ...
    cc = sender
    string = str("邮箱：%s 登陆授权码：%s 主题：%s " %(sender, passwd, title))
    print("邮箱：%s 登陆授权码：%s 主题：%s " %(sender, passwd, title))
...
for email in email_sy:
    ...
    if allnum < 70:
        ...
        if stat == 1:
            ...
            print('发送成功')
            dict_email[email] = '发送成功'
            c = c +1
        else:
            print(' 发送失败')
            dict_email[email] = '发送失败'
            d = d +1
    else:
        continue
ba = c + d
